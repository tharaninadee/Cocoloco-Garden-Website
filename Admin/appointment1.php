...
<br><br><br>
<div class="topic"><img class="coco" width="80" height="50" style="margin-right:100px; margin-bottom:10px; margin-left:30px;" role="img" src="../Image/loco.png">Customer Booking - Time Slot (11.00am - 12.00pm) 
<button class="btn btn-brown" style="margin-bottom:10px; margin-left:100px;" onclick="goBack('appointment1.php')">REFRESH</button>
...
if (isset($_POST['search'])) {
    ...
    $query = $conn->query("SELECT * FROM `appointment` WHERE $conditionStr ORDER BY date DESC") or die(mysqli_error());
} else {
    // Add ORDER BY clause to sort by date in descending order
    $query = $conn->query("SELECT * FROM `appointment` ORDER BY date DESC") or die(mysqli_error());
}
...
?>
    <tr>
        ...
        <td>
            <a class="delete-btn" onclick="confirmationDelete(this); return false;" href="../Delete/delete_appointment1.php?id=<?php echo $fetch['Id'] ?>">
                Delete
            </a>
        </td>
    </tr>
